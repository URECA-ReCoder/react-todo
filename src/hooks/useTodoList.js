...
export function useTodoList() {
    ...
    const moveToCompletedForSelected = () => {
        ...
        const newActiveItems = activeItems.filter((_, index) => !selectedItems.includes(index));
    ...
    const moveToTodoListForSelected = () => {
        const itemsToMove = completedItems.filter((_, index) => selectedItems.includes(index));
        const plainItems = itemsToMove.map((item) => item.text);
        const newCompletedItems = completedItems.filter((_, index) => !selectedItems.includes(index));
    ...
    const moveToCompleted = (index) => {
        ...
        const styledItem = { text: itemToMove, style: { textDecoration: 'line-through', color: '#888' } };
